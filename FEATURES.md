...
| Multi-core chunk building | Tessellate chunk geometry on worker threads | **in progress** | Phase 1: total `Tessellator.instance` redirect (78 sites, verified). Phase 2 groundwork merged: per-thread tessellator binding (soak-verified behavior-neutral) plus the unit-tested build queue and worker pool spine (stamps, generations, prioritization, failure discard). WorldRenderer's private cached-tessellator static - the last thread-coupling in the tessellation path - is also redirected (soak-verified). Remaining: the capture/replay wiring that runs builds on the workers. |
