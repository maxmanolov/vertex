...
| Multi-core chunk building | Tessellate chunk geometry on worker threads | **in progress** | Phase 1: total `Tessellator.instance` redirect (78 sites, verified). Phase 2 groundwork merged: per-thread tessellator binding (soak-verified behavior-neutral) plus the unit-tested build queue and worker pool spine (stamps, generations, prioritization, failure discard). Remaining: the WorldRenderer geometry/GL split that wires them together. |
