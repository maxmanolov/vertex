...
| Feature | Purpose | Vertex status | Notes |
|---|---|---|---|
| Connected textures (CTM) | Seamless glass/bookshelves etc. | **in progress** | Core merged; the shared icon dispatch it was to build on is **not yet trustworthy** (inconsistent invocation between runs - docs/OPEN-QUESTIONS.md). Blocked on resolving that before the ruleset loader and blob dispatch are worth writing. |
...
| Better grass / better snow | Side-grass and snow-under-fence rendering | **in progress** | Implementation merged and config-gated (default off), but **status corrected down from matched**: the icon dispatch it depends on fires inconsistently between runs (one instrumented soak measured 164k resolutions/min and armed the feature; a later soak under identical churn measured zero and never armed). Unexplained - see docs/OPEN-QUESTIONS.md. Not trustworthy until the dispatch is deterministic. |
