...
| Multi-core chunk building | Tessellate chunk geometry on worker threads | **in progress - replay defect found** | Corrected status: the dark-launch era's "gate-passed" telemetry counted hollow builds - a flag race made every worker body no-op, so replays swapped empty lists and no counter noticed (docs/benchmarks/multicore-status.md). The race is fixed and builds now execute, but the first capture that could actually see multicore's output shows fragmentary section geometry (docs/benchmarks/multicore-replay-fragments.png). Default remains OFF; do not enable for normal play. The capture harness that caught this is the regression gate for the fix. |
