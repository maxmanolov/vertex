...
| Custom colors | Pack-defined colormaps | **in progress** | Core + pack loading merged (0.2.0): the runtime reload listener (dynamic proxy, no bytecode) probes and parses color.properties and grass/foliage colormaps on every resource reload, soak-verified. Remaining: the colorizer hook that consumes them. |
