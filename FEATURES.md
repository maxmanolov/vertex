...
| Custom sky | Pack-defined sky boxes | **in progress** | Timing core **and layer loader** merged (0.2.0): numbered skyN.properties probed on every resource reload, each parsed into an immutable layer (source fallback, fade window, blend, rotation axis and speed) with malformed layers skipped rather than fatal. Remaining: the sky-pass render hook that draws the layers - the only piece needing new GL. |
