...
| Feature | Purpose | Vertex status | Notes |
|---|---|---|---|
| Connected textures (CTM) | Seamless glass/bookshelves etc. | **in progress** | Connectivity engine, format parser, **and the pack loader** merged (0.2.0): directory and zip packs are enumerated for arbitrary mcpatcher/ctm/*.properties names, rules indexed for O(1) render-path lookup, one bad file never costs a pack its others. Remaining: registering pack-supplied tile sprites into the block atlas (needs a TextureMap hook - see docs/ROADMAP.md #2), then blob-class dispatch on the verified icon hook. |
