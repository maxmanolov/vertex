...
- Transformers are exercised end-to-end against the real Mojang 1.7.10 jar. The built-in
  ...
  root-caused #118), `-Dvertex.profileServer=true` reports the integrated server's tick-time distribution (p50/p95/p99/max plus a count of ticks over the 50ms budget) every 10 seconds as `[VertexSrv]` - percentiles rather than means, because server pacing claims live entirely in the tail, and
